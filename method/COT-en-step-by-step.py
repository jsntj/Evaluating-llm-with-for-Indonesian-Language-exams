...
This version (COT-en-step-by-step.py) outputs a simplified CSV with only:
- question
- ground_truth
- prediction
- reasoning
...
Please think step by step and explain your reasoning for this question. First, carefully read and understand what the question is asking. Then, analyze each choice option systematically and evaluate how well it addresses the question. Consider the key concepts, facts, or principles that relate to each option. After completing your step-by-step analysis, provide your final answer in the format "Final Answer: X" where X is the letter (A, B, C, D, or E)."""

def extract_answer(response):
    """
    Extract answer letter (A-E) from model response, prioritizing 'Final Answer:' pattern.
    
    This function analyzes the model's output to extract the predicted answer.
    The full response contains the reasoning chain, and this function extracts
    just the final answer letter.
    
    Args:
        response: Full model response containing reasoning and answer
...
def extract_reasoning(response):
    ...
    The reasoning is the full response text, which contains the structured
    reasoning steps (Step 1-4) and conclusion. This function can be used
    to clean or format the reasoning if needed, but by default returns
    the full response as-is.
...
def main():
    ...
    txt_output_path = os.path.join(results_dir, "COT_en_step_by_step_summary.txt")
    csv_output_path = os.path.join(results_dir, "COT_en_step_by_step_results.csv")
    latex_output_path = os.path.join(results_dir, "COT_en_step_by_step_results.tex")
