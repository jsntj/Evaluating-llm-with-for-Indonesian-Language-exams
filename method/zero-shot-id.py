...
def get_subject_level_groups(csv_path, num_groups=20, ensure_subject_diversity=True):
    """
    Get top N subject-level groups from the IndoMMLU dataset, ensuring subject diversity
...
def format_prompt(question, choices):
    """Format pertanyaan dan pilihan menjadi prompt untuk model."""
...
def save_results_to_latex(all_model_results, output_path):
    ...
    latex_content.append("\\title{Zero-Shot Evaluation Results (Indonesian)}")
    ...
    summary_path = os.path.join(results_dir, "results_zero_shot_id_summary.txt")
    csv_path = os.path.join(results_dir, "results_zero_shot_id_results.csv")
    latex_path = os.path.join(results_dir, "results_zero_shot_id_results.tex")
