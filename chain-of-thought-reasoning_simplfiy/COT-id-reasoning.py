...
This version (COT-en-reasoning.py) outputs a simplified CSV with only:
- question
- ground_truth
- prediction
- reasoning
...
def format_prompt(question, choices):
    """Format the question and choices with a concise Chain of Thought prompt."""
...
def extract_answer(response):
    ...
    Fungsi ini menganalisis keluaran model untuk mengekstrak jawaban yang diprediksi.
    Respons lengkap berisi rantai penalaran, dan fungsi ini mengekstrak
    hanya huruf jawaban akhir.
    
    Args:
        Respons: Respons model lengkap yang mencakup alasan dan jawaban.
    
    Returns:
        Surat balasan (A-E) atau Tidak ada jika tidak ditemukan
    """
...
def extract_reasoning(response):
    """
    Ekstrak rantai penalaran dari respons model.
    Penalaran adalah teks respons lengkap yang berisi langkah-langkah
    penalaran terstruktur (Langkah 1-4) dan kesimpulan. Fungsi ini dapat
    digunakan untuk membersihkan atau memformat penalaran jika diperlukan,
    tetapi secara default mengembalikan respons lengkap apa adanya.
    
    Args:
        response: Full model response
    
    Returns:
        Teks penalaran (jawaban lengkap)
    """
...
def main():
    ...
    csv_output_path = os.path.join(script_dir, "COT_en_reasoning_2_results.csv")
    latex_output_path = os.path.join(script_dir, "COT_en_reasoning_2_results.tex")
